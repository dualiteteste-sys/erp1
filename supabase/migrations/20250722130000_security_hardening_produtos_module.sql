/*
  # [Security Hardening] Módulo de Produtos
  Este script corrige os avisos de segurança reportados, aumentando a robustez do banco de dados.
  ## Descrição da Query:
  - **Segurança**: Move a extensão `moddatetime` para um schema dedicado `extensions`, conforme as boas práticas do Supabase, para corrigir o aviso "Extension in Public".
  - **Segurança**: Recria as funções RPC (`create_produto_completo`, `update_produto_completo`, `delete_produto`) definindo um `search_path` explícito. Isso corrige o aviso "Function Search Path Mutable", prevenindo potenciais ataques de sequestro de caminho de busca (path hijacking).
  - **Manutenção**: Os gatilhos (`triggers`) que usam `moddatetime` são recriados para apontar para a nova localização da função no schema `extensions`.
  - Esta operação é segura e não afeta os dados existentes.
  ## Metadados:
  - Schema-Category: "Structural"
  - Impact-Level: "Low"
  - Requires-Backup: false
  - Reversible: true (revertendo as alterações)
  ## Implicações de Segurança:
  - RLS Status: Sem alterações.
  - Policy Changes: Não.
  - Auth Requirements: Requer permissões de superusuário para alterar extensões e funções.
*/

-- 1. Criar o schema 'extensions' se ele não existir.
CREATE SCHEMA IF NOT EXISTS extensions;

-- 2. Mover a extensão 'moddatetime' para o schema 'extensions'.
-- Isso corrige o aviso "Extension in Public".
ALTER EXTENSION moddatetime SET SCHEMA extensions;

-- 3. Recriar os gatilhos (triggers) para usar a função no novo schema.
...
-- Re-create triggers pointing to the new schema
...
-- 4. Recriar as funções RPC com um search_path explícito.
-- Isso corrige o aviso "Function Search Path Mutable".
...
LANGUAGE plpgsql
SECURITY DEFINER
-- FIX: Define o search_path para maior segurança
...
CREATE OR REPLACE FUNCTION public.delete_produto(
    p_id uuid
)
RETURNS text[]
LANGUAGE plpgsql
