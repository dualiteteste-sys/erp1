/*
  ...
  Este script ajusta as funções do módulo de produtos para seguir as melhores práticas de segurança, corrigindo os avisos de "Function Search Path Mutable".
  ## Descrição da Query:
  - **Segurança**: Altera as funções RPC existentes (`create_produto_completo`, `update_produto_completo`, etc.) para definir um `search_path` explícito. Isso previne potenciais ataques de sequestro de caminho de busca.
  - **Auditoria**: Garante que as colunas `created_by` e `updated_by` sejam preenchidas corretamente, usando o `auth.uid()` do usuário autenticado.
  - **Idempotente**: Usa `CREATE OR REPLACE` para atualizar as funções sem causar erros se o script for executado mais de uma vez.
  ## Metadados:
  - Schema-Category: "Safe"
  - Impact-Level: "Low"
  - Requires-Backup: false
  - Reversible: true (revertendo para a versão anterior das funções)
*/

-- Adiciona as colunas de auditoria se elas não existirem, garantindo idempotência.
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='produtos' AND column_name='created_by') THEN
    ALTER TABLE public.produtos ADD COLUMN created_by uuid REFERENCES auth.users(id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='produtos' AND column_name='updated_by') THEN
    ALTER TABLE public.produtos ADD COLUMN updated_by uuid REFERENCES auth.users(id);
  END IF;
END;
$$;

-- Atualiza a função de criação de produto para incluir auditoria e search_path seguro.
...
-- Atualiza a função de update de produto para incluir auditoria e search_path seguro.
...
-- Atualiza a função de deleção de produto para incluir search_path seguro.
...
-- Atualiza a função de criação de imagem de produto para incluir search_path seguro.
...
BEGIN
    ...
    RETURNING id INTO v_new_imagem_id;

    SELECT json_build_object('id', id, 'produto_id', produto_id, 'storage_path', storage_path, 'nome_arquivo', nome_arquivo, 'created_at', created_at)
    INTO v_result
    FROM public.produto_imagens
    WHERE id = v_new_imagem_id;
