/*
          ...
          [This script robustly creates the 'situacao_servico' type, adds new columns to the 'servicos' table, and recreates its related functions, fixing the default value casting error.]
          ## Query Description: [This operation corrects a previous migration error by dropping the old default value before altering a column type. It then adds new columns and recreates the necessary functions. It is designed to be safe and should not affect existing data, but a backup is always recommended.]
          ## Metadata:
...
-- Step 4: Correct the 'situacao' column type.
DO $$
BEGIN
    IF EXISTS (
        ...
        ALTER TABLE public.servicos ALTER COLUMN situacao DROP DEFAULT;
