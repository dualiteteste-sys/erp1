/*
          # [Fix and Enhance Servicos Module v3]
          [This script robustly creates the 'situacao_servico' type, ensures the 'servicos' table schema is correct, and recreates the necessary RPC functions for creating and updating services.]
          ## Query Description: [This is a comprehensive script to fix the 'servicos' module. It defensively creates types and tables, adds missing columns, and recreates stored procedures. This approach is safe and idempotent, meaning it can be run multiple times without causing errors. It should not affect existing data, but a backup is always recommended.]
          ## Metadata:
...
-- Step 2: Ensure the 'servicos' table exists.
CREATE TABLE IF NOT EXISTS public.servicos (
    id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
    empresa_id uuid NOT NULL REFERENCES empresas(id) ON DELETE CASCADE,
    descricao text NOT NULL,
    preco numeric(10, 2) NOT NULL DEFAULT 0,
    situacao text NOT NULL DEFAULT 'Ativo', -- Use text temporarily to avoid errors
...
-- Step 4: Alter the 'situacao' column to the correct ENUM type. This is the fix for the user's error.
DO $$
BEGIN
    IF EXISTS (
