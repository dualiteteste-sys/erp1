/*
# [SECURITY] Fix Function Search Path for create_produto_v2 (uuid version)
[This operation secures the function by setting a fixed search_path, preventing potential hijacking attacks.]

## Query Description: [Updates the function `create_produto_v2(uuid, jsonb, jsonb, jsonb)` to explicitly set the `search_path` to `public`. This is a security best practice that ensures the function always references objects in the correct schema, mitigating risks of unauthorized code execution. This change does not affect data or application logic.]
...
## Structure Details:
- Function: public.create_produto_v2(uuid, jsonb, jsonb, jsonb)
...
begin
  ...
  -- jsonb_to_record: converte o JSON do app em colunas
  with d as (
    select *
  ...
  -- relacionamentos (se existirem as tabelas e as FKs)
  -- atributos
...
/*
# [SECURITY] Fix Function Search Path for create_produto_v2 (jsonb version)
[This operation secures the function by setting a fixed search_path, preventing potential hijacking attacks.]

## Query Description: [Updates the function `create_produto_v2(jsonb, jsonb, jsonb)` to explicitly set the `search_path` to `public`. This is a security best practice that ensures the function always references objects in the correct schema, mitigating risks of unauthorized code execution. This change does not affect data or application logic.]
...
## Structure Details:
- Function: public.create_produto_v2(jsonb, jsonb, jsonb)
...
- Estimated Impact: [Negligible performance impact.]
*/
CREATE OR REPLACE FUNCTION public.create_produto_v2(p_produto_data jsonb, p_atributos jsonb, p_fornecedores jsonb)
...
begin
  SET search_path = public;
  -- insere linha básica (empresa_id é obrigatório e vem em p_produto_data)
  insert into public.produtos(
